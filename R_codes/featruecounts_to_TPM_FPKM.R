...
counts_file = '/Users/jplab/Desktop/2022-2/code/2-22_counts_to_TPM_FPKM/2umol.read.counts.csv'
# args <- commandArgs(T)
# tag <- tools::file_path_sans_ext(args[1])
tag <- tools::file_path_sans_ext(counts_file)

ftr.cnt <- read.csv(counts_file,header = T, sep=",",stringsAsFactors = F)
# (counts_file, sep="\t", stringsAsFactors=FALSE,
#                      header=TRUE)
library(dplyr)
library(tidyr)
