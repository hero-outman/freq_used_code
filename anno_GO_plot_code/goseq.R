...
# must load this package, or dbgetquery() method will not be found, then error
library(DBI)


setwd("/Users/jplab/Desktop/ATAC_data/annotation/")

filenames = list.files(path="./anno_tsv/", pattern="*.tsv$")

filenames

#all = read.delim("all_genes.tsv")$ensg
all = read.delim("./hg38_ensembl_v95_enst.tsv")$ensg

# duplicated(all)

all <- all[!duplicated(all)]
#all_no_dup <- all[!duplicated(all)]

for (filename in filenames) {
  print(filename)
  
  data = read.delim(paste("./anno_tsv/", filename, sep=""), row.names=1) # id for deseq
  ensg = rownames(data)
  
  vec = as.integer(all%in%ensg)
  names(vec) = all
  
  pdf(paste("barplots.", filename, ".pdf", sep=""), width=6, height=3.2)
  par(mar=c(4, 14, 3, 3)) 
  ...
  for (ont in c("CC", "BP", "MF")) {
    ...
    if (dim(this_ont)[1] > 1) {
      ...
      write.table(tab, paste("./GO/tabs/tab", ont, '_', filename, sep=""), sep="\t", col.names=NA)
    }
